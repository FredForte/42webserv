...
static std::string extractUploadFilename(const LocationConfig& location,
    ...
    std::string filename =
        (last_slash == std::string::npos) ? relative : relative.substr(last_slash + 1);
...
// getErrorPage needs.
static std::string generateAutoIndexHtml(const std::string& directory_path,
...
HttpResponse buildRedirectResponse(ServerConfig& server, LocationConfig& location,
    ...
    std::stringstream body;
    body << "<html>\n<head><title>" << response.code << " " << response.description
         << "</title></head>\n"
         << "<body>\n<center><h1>" << response.code << " " << response.description
         << "</h1></center>\n"
